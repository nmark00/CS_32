...
void diffMaker(hashTable m_hashTable, string foldString, string fnewString, string& diff)
{
    ...
    {
        if (newIndex >= fnewString.size() - (nsize-1))//add the last 1-3 char
        {
        ...
        string hashMe = fnewString.substr(newIndex, nsize);//create miniString
...
//            addMe += hashMe;
        ...
        if (newIndex +nsize >= fnewString.size())//make sure newIndex is small enough
            continue;//break;
