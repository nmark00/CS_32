...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    //       parameter comp.
    int i, j;
    for (i = 1, j = i-1; i< s.size(); i++)
    {
        Sensor key = s[i];
        while (i> 0 && comp(s[i], s[j]))
        {
            s[i] = s[j];
            j--;
        }
